    ...
    public partial class TabelaClienteControl : UserControl
    {
        ...
        public void AtualizarRegistros(List<Cliente> clientes)
        {
            ...
            foreach (Cliente cliente in clientes)
            {
                ...
                {
                    grid.Rows.Add(cliente.Id, cliente.Nome, cliente.Endereco, double.Parse(cliente.Telefone), cliente.Email, double.Parse(cliente.CPF));
                    this.grid.Columns[5].DefaultCellStyle.Format = @"000\.000\.000\-00";
                }
                else
                {
                    grid.Rows.Add(cliente.Id, cliente.Nome, cliente.Endereco, double.Parse(cliente.Telefone), cliente.Email, double.Parse(cliente.CNPJ));
                    this.grid.Columns[5].DefaultCellStyle.Format = @"00\.000\.000\/0000\-00";
                }
